...
const handleOrderTypeLimit = (req: Request, res: Response, userId: string, stockSymbol: string, type: string, side: string, userPrice: number, quantity: number, collateral:number) => {
	...
	if(
		...
		if(PERPETUAL_ORDERBOOK_STORE[stockSymbol].long[userPrice]){
			...
			//create long
			actionCreateLong(userId, stockSymbol, userPrice, quantity)
			return res.json(new HttpSuccessResponse(200, true, "Order Placed", PERPETUAL_ORDERBOOK_STORE[stockSymbol]));
		}
	}
...
const handlePriceNotAvailableInLimitOrder = (req: Request, res: Response, userId: string, stockSymbol: string, type: string, side: string, userPrice: number, userQuantity: number, collateral: number) => {
	...
	for(const price of PERPETUAL_ORDERBOOK_STORE_INDEX[stockSymbol].short){
		...
		if(price > userPrice &&  fullfilledQuantity != userQuantity){
			actionCreateLong(userId, stockSymbol, userPrice, (userQuantity - fullfilledQuantity));
			break;
		}
		...
		if(shortInfo?.remainingQuantity > (userQuantity - fullfilledQuantity) ){
			...
			PERPETUAL_ORDERBOOK_STORE[stockSymbol].short[price]?.orders.push({
				userId,
				quantity:shortInfo.totalQuantity,
	...
	console.log("CONTRACT STORE", CONTRACT_STORE)

	return res.json(new HttpSuccessResponse(200, true, "Order Placed", PERPETUAL_ORDERBOOK_STORE[stockSymbol]));
}

const actionCreateLong = (userId:string, stockSymbol:string, userPrice:number, quantity:number) => {
